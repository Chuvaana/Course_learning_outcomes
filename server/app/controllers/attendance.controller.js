            ...
            bulkOperations.push({
                updateOne: {
                    ...
                    update: {
                        ...
                        $addToSet: {  // Ensure we do not add duplicate student attendance
...
exports.getAttendanceByLesson = async (req, res) => {
    try {
        ...
        const attendanceRecords = await Attendance.find({ lessonId: id }).populate('attendance.studentId', 'studentName studentCode');
        res.json(attendanceRecords);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
