...
export class AttendanceService {
    ...
    createAttendanceAll(attendance: Attendance[]): Observable<Attendance> {
        return this.http.post<Attendance>(`${this.apiUrl}/all`, { attendanceDatas: attendance });
    }

    // ✅ Get attendance records by filters (lessonId, weekNumber, type)
    getAttendance(lessonId: string, weekDay?: string, type?: string, time?: number): Observable<Attendance[]> {
