...
export class ProgressPollComponent {
  ...
  cloQuestion() {
    if (!this.createActive) {
      this.cloService.getCloList(this.lessonId).subscribe((e: any) => {
        ...
        answerData.push({
          answerName:
            'Оюутан та энэ хичээлээр эзэмшсэн мэдлэг, чадвар, хандлагын түвшингээ тодорхойлно уу',
          answerId: 0,
          answerValue: 0,
          questionType: { code: 'RATE', name: 'Үнэлгээ өгөх' },
          questionTypeName: 'Үнэлгээ өгөх',
        });
  ...
  addAnswer(questionIndex: number) {
    this.dataQuestions[questionIndex].answers.push({
      answerName: '',
      answerId: 1,
      answerValue: 0,
      questionType: 'RATE',
